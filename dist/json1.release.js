...
function insertChildMut(container, key, value) {
    return "number" == typeof key ? (assert(Array.isArray(container), "Cannot use numerical key for object container"), 
...
function transformPosition(path, op) {
    ...
    const handleDrop = () => {
        let i = 0;
        for (null != pickedAtSlot && (i = pickIndex, path = r.getPath().concat(path.slice(pickIndex))); i < path.length; i++) {
            const k = path[i], c = getComponent(r), et = getEditType(c);
            if (et) {
                const e = getEdit(c);
                et.transformPosition ? path[i] = et.transformPosition(path[i], e) : et.transformCursor && (path[i] = et.transformCursor(path[i], e));
                break;
            }
            let dropOffset = 0;
...
function transformPresence(presence, op) {
    if (!presence || !presence.start || !presence.end) return null;
    const start = transformPosition(presence.start, op), end = transformPosition(presence.end, op);
    return start && end ? Object.assign(Object.assign({}, presence), {
        start: start,
        end: end
    }) : start ? Object.assign(Object.assign({}, presence), {
        start: start,
        end: start
    }) : end ? Object.assign(Object.assign({}, presence), {
        start: end,
        end: end
    }) : null;
}
